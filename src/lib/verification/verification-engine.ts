...
export function generateVerificationReport(report: FullVerificationReport): string {
  ...
  lines.push("");
  lines.push("External reference values are intentionally left as `null` (pending) until generated from R psych, Jamovi, or SPSS. No expected values in this report are AI-generated or fabricated. Values marked as `pending` must be populated by running the reference datasets through the specified external tools before verification comparisons can be made.");
  lines.push("");
  lines.push("---");
