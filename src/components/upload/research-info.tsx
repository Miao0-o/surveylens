  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-1.5">
        <label className="text-sm font-medium text-foreground">研究信息</label>
        <span className="text-xs text-muted-foreground">选填</span>
      </div>

      {/* Research goal */}
      <div className="space-y-1">
        <label className="text-sm text-muted-foreground">研究目标</label>
        <textarea
          value={researchGoal}
          onChange={(e) => setResearchGoal(e.target.value)}
          placeholder="例如：测量大学生焦虑水平"
          rows={2}
          className="w-full resize-none rounded-lg border border-border bg-background px-3 py-2.5 text-sm text-foreground placeholder:text-muted-foreground/50
      ...
      <div className="space-y-1">
        ...
        <input
          ...
          className="w-full rounded-lg border border-border bg-background px-3 py-2.5 text-sm text-foreground placeholder:text-muted-foreground/50
            focus:outline-none focus:ring-1 focus:ring-ring focus:border-ring transition-colors"
        />
      </div>
