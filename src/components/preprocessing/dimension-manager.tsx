...
export function DimensionManager() {
  ...
  // Items already assigned to a dimension
  ...
  // Auto-create dimensions from theoretical input
      ...
      <div className="space-y-2 max-h-[300px] overflow-y-auto">
        {dimensions.map((dim) => (
          ...
          >
            ...
            {/* Dimension items */}
            <div className="p-1.5 space-y-0.5 min-h-[28px]">
              {dim.items.length === 0 ? (
          ...
          <div className="flex flex-wrap gap-1">
            {unassignedItems.map((item) => (
              <div key={item} className="relative group">
                <button
                  onClick={() => {
                    // Assign to first available dimension, or prompt to create one
                    if (dimensions.length > 0) {
                      addItemToDim(dimensions[0].name, item);
                    }
                  }}
                  className="px-2 py-0.5 rounded border border-border bg-background text-[11px] text-muted-foreground
                    hover:text-foreground hover:border-primary/30 transition-colors"
                >
                  {item}
                </button>
                {/* Dropdown for dimension selection */}
                {dimensions.length > 1 && (
                  <div className="hidden group-hover:flex absolute top-full left-0 mt-0.5 z-10 flex-col bg-card border border-border rounded-lg shadow-sm py-0.5 min-w-[120px]">
                    {dimensions.map((dim) => (
                      <button
                        key={dim.name}
                        onClick={() => addItemToDim(dim.name, item)}
                        className="px-2.5 py-1 text-[10px] text-muted-foreground hover:text-foreground hover:bg-secondary/50 text-left"
                      >
                        分配到「{dim.name}」
                      </button>
                    ))}
