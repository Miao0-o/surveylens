...
function skewLabel(s: number | null): string {
  if (s === null) return "-";
  if (Math.abs(s) < 0.5) return "对称";
  if (Math.abs(s) < 1) return "轻度偏态";
  return "偏态";
}
...
export function DescriptiveCard({ data, labels }: Props) {
  ...
  return (
    <div className="space-y-4">
      <div>
        <h3 className="text-xs font-medium text-foreground mb-1">描述性统计与正态性检验</h3>
        <p className="text-[10px] text-muted-foreground">
          Shapiro-Wilk 正态性检验 | 偏度 (Skewness) | 峰度 (Kurtosis)
        </p>
      </div>
        ...
        <table className="w-full text-[10px]">
          <thead>
            <tr className="bg-secondary/50 sticky top-0">
              <th className="text-left px-2.5 py-1.5 text-muted-foreground font-medium border-r border-border">变量</th>
              ...
              <th className="text-right px-2.5 py-1.5 text-muted-foreground font-medium border-r border-border">偏度</th>
              <th className="text-right px-2.5 py-1.5 text-muted-foreground font-medium border-r border-border">峰度</th>
              <th className="text-center px-2.5 py-1.5 text-muted-foreground font-medium">正态性</th>
            </tr>
          </thead>
          <tbody>
            {data.map((row, i) => (
              <tr key={i} className="border-t border-border hover:bg-secondary/20">
                ...
                <td className={`text-right px-2.5 py-1.5 border-r border-border ${row.skew !== null && Math.abs(row.skew) > 1 ? "text-amber-600" : "text-foreground"}`}>
                  {row.skew?.toFixed(2) ?? "-"} <span className="text-[8px] text-muted-foreground">{skewLabel(row.skew)}</span>
                </td>
